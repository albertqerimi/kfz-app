...
$params = [];
$param_types = 's'; // Initial type for search
$search_like = '%' . $search . '%'; // Prepare the search pattern
$params[] = $search_like; // Add search pattern to params
...
if ($filter_date_from && $filter_date_to) {
    ...
    $param_types .= 'ss'; // Adding types for date
} elseif ($filter_date_from) {
    $invoice_sql .= " AND invoices.date >= ?";
    $params[] = $filter_date_from;
    $param_types .= 's'; // Adding type for date
...
// Add pagination
$invoice_sql .= " ORDER BY invoices.id DESC LIMIT ? OFFSET ?";
$params[] = $items_per_page;
$params[] = $offset;
$param_types .= 'ii'; // Adding types for limit and offset

$stmt = $conn->prepare($invoice_sql);
$stmt->bind_param($param_types, ...$params); // Use the splat operator to unpack the params
...
<div class="row">
    <div class="col-12">
        ...
        <form method="GET" action="list_invoices.php" class="form-inline mb-3 justify-content-center">
